    ...
    for t in range(T):
        ...
        for e in range(E[t]):
            ...
            for i in range(I[t][e]):
                ...
                #jobs.append(pool.apply_async(img2net.help.grid_all,args=(inp,)))
            #for job in jobs:
                #res=job.get()
                res=img2net.help.grid_all(inp)
                graphn[res[2]]=res[3]
            ...
            #for job in jobs:
                #res=job.get()
                res=img2net.help.graph_all(inp)
...
################################################################# save data
    ...
    for l,label in enumerate(labels):
        ...
        plt.subplot(121)
        plt.suptitle(label)
        img2net.help.boxplot(dn,'black',treatments,0)
        plt.ylabel('absolute')
        plt.xlabel('treatments')
        plt.subplot(122)
        plt.plot([0,T+1],[1,1],lw=2,color='gray',ls='--')
        img2net.help.boxplot(dd,'black',treatments,1)
        plt.ylabel('relative')
        plt.xlabel('treatments')
        plt.tight_layout()
    ...
    for t in range(T):
        for e in range(E[t]):
            ...
            pos2D,pos3D=img2net.help.grid_pos2D(N,lz,posi)
